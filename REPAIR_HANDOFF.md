...
**Branch:** `DO-NOT-USE-vincent-repair-unreviewed` (pushed to this repo as review material only — the name is the warning). Base: upstream `b84c83a` "Footprint and symbol fix".
...
4. `91ecfe2` **Production/ regenerated** — everything from one board revision, using the board's saved plot params (identical 19-file gerber recipe + zip). New: `bom.csv` (replaces stale Jun-15 `bom.xlsx`; R5 LCSC consolidated to C413111 = R6/R7's SKU, same 10K 0402) and `HD3SS3220-breakout-cpl.csv` (pick-and-place, previously missing). Fresh IPC-D-356 `netlist.ipc`. *Note: the two CSVs were silently excluded by the repo's `*.csv` gitignore rule and landed in a follow-up commit (force-added); caught by a clean-clone rebuild check.*
...
Diff is reviewable per commit: `git log -p b84c83a..DO-NOT-USE-vincent-repair-unreviewed`. If upstream wants it: cherry-pick or merge after review — do not merge or fabricate from this branch as-is (see banner). Note `.gitignore` excludes `*.csv`; the BOM/CPL are force-added, so future regenerations must also use `git add -f`.
